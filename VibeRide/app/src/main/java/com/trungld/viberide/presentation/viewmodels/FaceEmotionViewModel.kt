...
class FaceEmotionViewModel @Inject constructor() : ViewModel() {
    ...
    private val _warningCount = MutableStateFlow<Int>(0)
    val warningCount: StateFlow<Int> = _warningCount

    // Sleepy detection for warning dialog
    private var sleepyStartTime: Long? = null
    private val sleepyDurationThreshold = 5000L // 5 seconds in milliseconds
    private val _showWarningDialog = MutableStateFlow(false)
    val showWarningDialog: StateFlow<Boolean> get() = _showWarningDialog
    ...
    fun updateEmotionFromFaceMesh(faceMesh: FaceMesh?) {
        ...
        // Check for sleepy state duration
        handleSleepyState(emotion)
    }
    ...
    fun inferEmotionForMusicAndSleepiness(face: FaceMesh): EmotionResult {
        ...
        // Sad detection
        if (isFrowning && frownIntensity > 0.002f) {
            sadScore += frownIntensity * 25f
        }
    ...
    private fun handleSleepyState(emotion: EmotionResult) {
        if (emotion.isSleepy) {
            if (sleepyStartTime == null) {
                sleepyStartTime = System.currentTimeMillis()
            } else {
                val duration = System.currentTimeMillis() - (sleepyStartTime ?: 0)
                if (duration >= sleepyDurationThreshold && !_showWarningDialog.value) {
